#!/bin/bash
# design-loop — Visual Intelligence Pyramid validation (4 tiers)
set -eo pipefail
PASS=0; FAIL=0; TOTAL=0
p() { PASS=$((PASS+1)); TOTAL=$((TOTAL+1)); }
f() { echo "  FAIL: $1"; FAIL=$((FAIL+1)); TOTAL=$((TOTAL+1)); }

# ═══ TIER 1: design-lint (instant, no browser) ═══
echo "▸ Tier 1: design-lint"
test -f skills/design-lint/SKILL.md && p || f "T1.01 skill exists"
test -f commands/design-lint.md && p || f "T1.02 command exists"
test -f orchestrator/lint-engine.md && p || f "T1.03 lint-engine exists"
grep -qi "lint-engine" skills/design-lint/SKILL.md && p || f "T1.04 skill→engine"
grep -qi "design-lint" commands/design-lint.md && p || f "T1.05 command name"
grep -qi "no browser" skills/design-lint/SKILL.md && p || f "T1.06 no browser claim"
...
grep -qi "design-score" orchestrator/lint-engine.md && p || f "T1.15 escalation to score"
grep -qi "SP-1\|SP-2" orchestrator/lint-engine.md && p || f "T1.16 rule IDs"

# ═══ TIER 2: design-score (5s, one screenshot) ═══
echo "▸ Tier 2: design-score"
test -f skills/design-score/SKILL.md && p || f "T2.01 skill exists"
test -f commands/design-score.md && p || f "T2.02 command exists"
test -f skills/modes/score/SKILL.md && p || f "T2.03 mode skill exists"
grep -qi "orchestrator" skills/design-score/SKILL.md && p || f "T2.04 skill→orchestrator"
grep -qi "score" skills/design-score/SKILL.md && p || f "T2.05 mode=score"
...
grep -qi "design-fix" skills/modes/score/SKILL.md && p || f "T2.08 escalation to fix"
...
# ═══ TIER 3: design-fix (30s, one iteration) ═══
echo "▸ Tier 3: design-fix"
test -f skills/design-fix/SKILL.md && p || f "T3.01 skill exists"
test -f commands/design-fix.md && p || f "T3.02 command exists"
test -f skills/modes/fix/SKILL.md && p || f "T3.03 mode skill exists"
grep -qi "orchestrator" skills/design-fix/SKILL.md && p || f "T3.04 skill→orchestrator"
grep -qi "fix" skills/design-fix/SKILL.md && p || f "T3.05 mode=fix"
...
grep -qi "design-loop" skills/modes/fix/SKILL.md && p || f "T3.13 escalation to loop"
...
grep -qi "design-score" orchestrator/lint-engine.md && p || f "T5.01 lint→score"
grep -qi "design-fix" orchestrator/lint-engine.md && p || f "T5.02 lint→fix"
grep -qi "design-loop" orchestrator/lint-engine.md && p || f "T5.03 lint→loop"
grep -qi "design-fix" skills/modes/score/SKILL.md && p || f "T5.04 score→fix"
grep -qi "design-loop" skills/modes/score/SKILL.md && p || f "T5.05 score→loop"
grep -qi "design-fix\|design-loop" skills/modes/fix/SKILL.md && p || f "T5.06 fix→loop"

# ═══ METADATA: Plugin Awareness ═══
echo "▸ Metadata: Plugin Awareness"
grep -qi "design-lint" CLAUDE.md && p || f "T6.01 CLAUDE.md knows lint"
grep -qi "design-score" CLAUDE.md && p || f "T6.02 CLAUDE.md knows score"
grep -qi "design-fix" CLAUDE.md && p || f "T6.03 CLAUDE.md knows fix"
grep -qi "pyramid\|4 tier" CLAUDE.md && p || f "T6.04 CLAUDE.md pyramid"
grep -qi "lint\|score\|4 tier\|pyramid" .claude-plugin/plugin.json && p || f "T6.05 plugin.json updated"
