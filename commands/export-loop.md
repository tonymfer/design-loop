...
0. Check if `.claude/design-loop-report.md` exists (generated by report-engine at loop completion):
   - If YES: output its contents, note "Report generated automatically at loop completion."
     Also note `.claude/design-loop-report.html` for browser viewing.
     Offer: "Run `/export-loop --regenerate` to rebuild from history data."
     STOP (unless `--regenerate` flag is present).
   - If NO: continue to step 1 (legacy fallback behavior).
...
The report-engine (`orchestrator/report-engine.md`) generates `.claude/design-loop-report.md` and `.claude/design-loop-report.html` automatically at loop completion (Step 8). It has access to live LOOP_STATE data including per-iteration fidelity scores, safety events, and brand fingerprint — producing a richer report than export-loop's history-file parsing.

**Coexistence:**
- If the report-engine has already generated a report, export-loop serves it directly (Step 0 above).
- If no report exists (e.g., older runs, manual cleanup), export-loop falls back to its history-parsing behavior.
- `--regenerate` forces a fresh build from history data even if a report-engine report exists.
